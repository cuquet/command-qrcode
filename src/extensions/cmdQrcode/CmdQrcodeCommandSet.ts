...
import QrcodeDialog from './CmdQrcode';
    ...
    switch (event.itemId) {
      case 'CMD_QR':
        ...
        if (event.selectedRows.length > 0) {
          ...
          fileName = (listFilter!=='')? list.title + strings.FilteredLabel: list.title;
        ...
        QRCode.toDataURL(absoluteUrl, { errorCorrectionLevel: 'H' })
        .then(url => {
          console.log(url);
          const dialog: QrcodeDialog = new QrcodeDialog();
            dialog.fileName = fileName;
            dialog.absolutePath = absoluteUrl;
            dialog.dataUri = url;
            dialog.show();
          })
          .catch(err => {
